    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            for j, target in enumerate(targets):
                if target[0] == i:
                    indices.append(j)
            image_targets = [targets[i] for i in indices]
            image_targets = torch.tensor(image_targets)  # Convert the list to a PyTorch tensor
            # No need to call cpu() method now, as image_targets is a tensor
            cpu_tensor = image_targets.cpu()
            image_targets_array = np.array(cpu_tensor)

            box_data = image_targets_array[:, 2:6]

            boxes = xywh2xyxy(box_data).T
            # 转换为数组
            image_targets_arr = np.array(image_targets)
            # 再切片
            classes = image_targets_arr[:, 1].astype('int')
            image_targets_arr = np.array(image_targets)
            gt = image_targets_arr.shape[1] == 6
...
def plot_lr_scheduler(optimizer, scheduler, epochs=120, save_dir=''):
